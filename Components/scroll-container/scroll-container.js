...
function scrollContainerByIndices(container, indices) {
    ...
    /* scrollLeft -1 because browsers report position with subpixels values, thus sometimes the child is wrongly identified without calculating -1 */
    let currentChild = children.find(child => child.offsetLeft >= containerEl.scrollLeft - 1);
    ...
    const updateMiniatureHighlight = (index) => {
        miniature.querySelectorAll('.described-item--miniature').forEach(item => {
            ...
            if (isSelected) {
                ...
                parent.scroll({
                    left: (item.offsetLeft - parent.offsetLeft) - (parent.offsetWidth / 2) + (item.offsetWidth / 2),
                    behavior: 'smooth'
                });
            }
        });
